% ANALYZE_STABILITY_B  Kiem tra nhanh do on dinh cua 20 lan chay B: so
% sanh Repository size va tap nghiem Pareto voi mat Pareto tham chieu that
% (Bruteforce_B_FINAL.mat).

clear; clc;
resultsDir = fullfile(fileparts(mfilename('fullpath')), 'results');
Sref = load(fullfile(resultsDir, 'Bruteforce_B_FINAL.mat'));
...
for r = 1:K
    f = fullfile(resultsDir, sprintf('MOSFOA2_B_Np8_Maxit20_run%02d_FINAL.mat', r));
    if ~isfile(f)
...
fprintf('\n=== TONG KET DO ON DINH - CONG TRINH B (20 lan chay) ===\n');
