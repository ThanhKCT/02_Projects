# -*- coding: utf-8 -*-
"""
Chay rieng phuong phap M2_NEW (TCVN 10304:2025, K=1850 kN/m4 tu Bang A.1 - Lop 2,
set deo chay) tren working model sap_work/Ben100kDWT_sensitivity.sdb.
...
KHONG chay lai. Script nay CHI:
  ...
  - Di chuyen 178 bottom_joint toi vi tri moi theo M2_NEW
  - RunAnalysis
  - Trich JointDispl (192 pile-top joints) + FrameForce (178 treatment frames), envelope BAO KT
  - Luu sap_work/results/joint_disp_M2_NEW.csv, pile_force_M2_NEW.csv
...
def main():
    ...
    new_coords = pd.read_csv(os.path.join(BASE_DIR, "new_bottom_coords_M2NEW.csv"))
    ...
    log("Unlocking + moving 178 bottom joints -> M2_NEW positions...")
    ...
    df_disp.to_csv(os.path.join(RESULTS_DIR, "joint_disp_M2_NEW.csv"), index=False)
    ...
    df_force.to_csv(os.path.join(RESULTS_DIR, "pile_force_M2_NEW.csv"), index=False)
