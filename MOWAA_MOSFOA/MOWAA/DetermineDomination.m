...
function pop=DetermineDomination(pop)% confirm dominance

    npop=numel(pop);
    
    for i=1:npop
        pop(i).Dominated=false; % initilization settment
        for j=1:i-1
            if ~pop(j).Dominated
                if Dominates(pop(i),pop(j))% compare two different solution
