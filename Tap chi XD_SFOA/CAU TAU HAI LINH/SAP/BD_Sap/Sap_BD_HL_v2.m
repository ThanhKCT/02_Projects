function [fit,g]=Sap_BD_HL_v2(X,data)
...
Angle_P = zeros(N_coc,1); Angle_P([2,4])=pi/2;
...
for ix = 1:size(X,1)
    ...
    lu = 2./alpha_e(layer);
    L_u = lu+L0; L_total = X(ix,11)+Pile_top(:,3); L_in_soil = L_total-L0;

    if L_total>=L_u
        D = zeros(N_coc,3); S = zeros(N_coc,3);
        Incline_P([2,4,6,7,8,9,13,14,15,16]) = X(ix,10);
        Angle_P([6,7,8,9,13,14,15,16])=X(ix,2:9);
