%% SOO_BD_run.m
% Single-objective SFOA (original algorithm) for the Berthing Dolphin (BD)
% structure. Adapted from the verified B-MOSFOA driver (BMOSFOA_BD_v3.m):
% same SAP2000-COM parallel worker infrastructure and Sap_BD_HL_v3 objective
...
%   objCol   = 1;              % 1 = Min Cost (case C1/BD-C), 2 = Min Displacement (case C2/BD-D)
...
load('X1_X2.mat'); % loads: data  (pile catalogue, see BD_Sap/Prestress_Pile_TCVN7888_2014.xlsx)
...
switch lower(runMode)
    ...
    case 'campaign'
        ...
        % Max_it=50 chosen from real pilot evidence: BD objective flat at
        % 10164.8212 from it=30 through it=110 (80 iters, no re-improvement)
        % -- see SOO_BD/results/BD_Cost_progress.log. 20-iter safety margin.
        Max_it = 50; Npop = 30;
    otherwise
...
logFile = fullfile(outDir, sprintf('BD_%s_progress.log', objName));
...
spmd (Num_work)
    ...
    workerFolder = fullfile(pwd,'BD_Sap', sprintf('SOO_%s_W%02d', objName, spmdIndex));
    if ~exist(workerFolder,'dir'); mkdir(workerFolder); end
    SM.File.Save('FileName', fullfile(workerFolder, sprintf('BD_%d.sdb', spmdIndex)));
...
for iloop = (1:Nrun) + runIdOffset
    ...
    saveName = fullfile(outDir, sprintf('BD_SOO_%s_%s_%s.mat', objName, runTag, upper(runMode)));
    ...
    fclose(flog);
    fprintf('[SOO-BD-%s] %s complete: best(penalized)=%.4f raw=%.4f elapsed=%.1fs\n', ...
        objName, runTag, fvalbest, BestDiag(objCol), RunResult.ElapsedSeconds);
end

system('taskkill /F /IM SAP2000.exe');
fprintf('[SOO-BD-%s] All %d run(s) complete. Results in %s\n', objName, Nrun, outDir);
