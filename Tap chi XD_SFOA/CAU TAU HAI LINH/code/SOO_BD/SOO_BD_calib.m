%% SOO_BD_calib.m
% NOT a paper-data driver. Hardware-calibration fork of SOO_BD_run.m: runs
...
% (seconds/FE) and confirm how many concurrent SAP2000 COM instances the
% node-locked license on this machine actually allows at Num_work workers.
% Do not use its .mat/.log output as paper data -- see SOO_BD_run.m for the
% real smoke/full driver.
%
...
switch lower(runMode)
    case 'calib'
        Max_it = 0;  Npop = 100; % single full-size batch eval; no SFOA loop
    otherwise
...
logFile = fullfile(outDir, sprintf('BD_%s_CALIB_progress.log', objName)); % separate from the real progress log
