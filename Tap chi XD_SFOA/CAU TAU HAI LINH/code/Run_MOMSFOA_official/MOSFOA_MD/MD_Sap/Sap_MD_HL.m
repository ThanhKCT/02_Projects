function [fit,g]=Sap_MD_HL(X,data)
...
for ix = 1:size(X,1)
    ...
    lu = 2./alpha_e(layer);
    L_u = lu+L0; L_total = L_u+X(ix,7); L_in_soil = lu+X(ix,7);
    ...
    for i = 1:4
        ...
        [S(i,:), ~]     = pile_create(Pile_top(i,:),     atan(1/X(ix,6)), X(ix,i+1),       [0,L_total],-1);
        [S(i+4,:), ~]   = pile_create(Pile_top(i+4,:),   atan(1/X(ix,6)), X(ix,i+1)+pi,    [0,L_total],-1);
    end
    [D(9,:), ~] = pile_create(Pile_top(9,:), atan(0), 0,[0,L_u],    -1);
    [S(9,:), ~] = pile_create(Pile_top(9,:), atan(0), 0,[0,L_total],-1);
