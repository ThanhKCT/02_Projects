...
function REP = updateGrid(REP,ngrid)
    ...
    ndim = size(REP.Fitness,2);
    REP.hypercube_limits = zeros(ngrid+1,ndim);
    for dim = 1:1:ndim
        REP.hypercube_limits(:,dim) = linspace(min(REP.Fitness(:,dim)),max(REP.Fitness(:,dim)),ngrid+1)';
    end
    
    % Computing where belongs each particle
    npar = size(REP.Fitness,1);
    REP.grid_idx = zeros(npar,1);
    REP.grid_subidx = zeros(npar,ndim);
    for n = 1:1:npar
        idnames = [];
        for d = 1:1:ndim
            REP.grid_subidx(n,d) = find(REP.Fitness(n,d)<=REP.hypercube_limits(:,d)',1,'first')-1;
