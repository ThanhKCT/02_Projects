%% SOO_MPJ_run.m
% Single-objective SFOA (original algorithm) for the Main Jetty Platform
% (MPJ/MJP) structure. Adapted from the verified B-MOSFOA driver
% (BMOSFOA_MPJ_v1.m): same SAP2000-COM parallel worker infrastructure and
% Sap_MPJ objective function, but WITHOUT any Pareto archive /
% repository / leader-selection mechanism. Only one objective column is
% optimized at a time (Cost or Displacement); the other is still logged
% for reporting.
%
% Design vector (nVar=6): x = [pile_type_idx, pile_length_L, span_X,
% span_Y, beam_width, beam_height] -- see BMOSFOA_MPJ_v1.m lb/ub/st.
%
% Usage (from MATLAB or `matlab -batch`):
%   objCol   = 1;              % 1 = Min Cost (case C5/MPJ-C), 2 = Min Displacement (case C6/MPJ-D)
...
logFile = fullfile(outDir, sprintf('MPJ_%s_progress.log', objName));
