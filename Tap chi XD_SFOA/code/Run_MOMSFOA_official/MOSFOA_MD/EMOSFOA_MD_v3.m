...
fprintf('[E-MOSFOA MD] Run mode: %s; Npop=%d; Max_it=%d; expected FEs=%d.\n', ...
...
ParallelConfig = struct( ...
    ...
    'PopulationSize', Npop);
fprintf('[E-MOSFOA MD] Using %d SAP2000 instances (%.0f%% of %d max logical workers; physical cores: %d).\n', ...
...
RunInfo.FEDefinition = 'One candidate evaluated by Sap_MD_HL_v3 counts as one FE.';
...
spmd (Num_work)
    ...
    Sap_name_new = sprintf('MD_%d.sdb', spmdIndex);
    workerFolder = fullfile(pwd,'MD_Sap', sprintf('W%02d',spmdIndex));
...
Name_res = sprintf('MD_EMOSFOA_%s_%s_WF%03d_Npop%d_Nvar%d_Maxit%d.mat', ...
