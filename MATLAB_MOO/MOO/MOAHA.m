function [ArchiveCost]=MOAHA(FunIndex,MaxIt,nPop,ArchiveBestSize)
...
for i=1:nPop
    ...
    Pop(i).Cost=BenFunctions(Pop(i).Position,FunIndex,Dim);
end
...
for It=1:MaxIt
    ...
    for i=1:nPop
        ...
        if rand<0.5   % Guided foraging
            ...
            newPop.Cost=BenFunctions(newPop.Position,FunIndex,Dim);
            for k=1:numel(F)
                if ismember(i,F{k})
                    iF=k;
                    break;
    ...
    if mod(It,2*nPop)==0 % Migration foraging
        ...
        for i=1:length(F{end})
            ...
            Pop(RandMaxDominatedIndex).Cost=BenFunctions(Pop(RandMaxDominatedIndex).Position,FunIndex,Dim);
        end
    end
...
ArchiveCost=[ArchiveBestPop.Cost];
