...
Container wharves on pile foundations require large quantities of pile material; selecting pile cross-sections at the preliminary design stage can be supported by constrained multi-objective optimization rather than relying on experience alone. This study formulates a multi-objective optimization problem for the cross-sections of prestressed spun concrete piles and steel pipe piles of a real 100,000-DWT container wharf, with a discrete design space of 4,080 combinations (5 concrete-pile catalogue classes × 51 steel-pile diameters × 16 steel-pile wall thicknesses) and a constraint set covering structural conditions, geotechnical bearing capacity per TCVN 10304:2025, and pile pull-out (uplift). The two objective functions — pile material mass and maximum lateral displacement — are evaluated with a SAP2000 finite-element model coupled to MATLAB via OAPI. The study compares two multi-objective optimization algorithms developed and reported by the same research group — MOFDA (flow-direction search with hybrid leader selection) and MOSFOA (starfish-inspired search; the experimental variant used here, E-MOSFOA, combines cosine phase control, leader-guided DE-mutation, and late-stage Gaussian refinement) — on the same problem, the same FEM model, the same objective functions, the same constraints, and the same finite-element-model evaluation budget (12,550 evaluations per run), with 30 independent runs per algorithm, benchmarked against a reference Pareto front obtained by exhaustively enumerating all 4,080 combinations (59 non-dominated solutions). In the problem studied, E-MOSFOA showed faster convergence and closer proximity to the reference Pareto front under the same FEM evaluation budget (mean IGD of 0.000100 versus 0.000506 for MOFDA; Wilcoxon rank-sum p = 1.07×10⁻¹⁰; reaching the hypervolume plateau after roughly 1,000–1,500 FEM evaluations, versus roughly 5,000–7,000 for MOFDA). In terms of coverage of the reference Pareto front, E-MOSFOA achieved a higher and more consistent rate than MOFDA (98.76% ± 0.88% versus 89.27% ± 4.46%, out of 59 reference solutions). The results provide a quantitative basis for algorithm selection when solving pile-cross-section design problems for port structures with complex constraints, within the scope of the problem, design domain, and computational model investigated in this study.
