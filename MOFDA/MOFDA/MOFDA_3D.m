...
addpath Functions
addpath(genpath('mathematical problems'))
for probnum= 1:8;
[obj,PF,nVar,obj_no,lb,ub,ref_point]= problem_3obj(probnum);
...
DOM2                    = checkDomination(fit); % 0 vt tri (tt hn); 1 b vt tri - (t hn)
REP{iloop}.pos                 = X(~DOM2,:); % Lu nhng c� th vt tri (tt hn)
REP{iloop}.pos_fit             = fit(~DOM2,:); % Lu nhng fitness vt tri (tt hn)
...
for i=1:Np
    for j=1:beta
        ...
        fit_nei(j,:)    = obj(X_nei(j,:))';
    end
    DOM1                   = checkDomination(fit_nei); % 0 vt tri (tt hn); 1 b vt tri - (t hn)
    REP_nei.pos                = X_nei(~DOM1,:); % Lu nhng c� th vt tri (tt hn)
    REP_nei.pos_fit            = fit_nei(~DOM1,:); % Lu nhng fitness vt tri (tt hn)
...
% save MOFDA_DTLZ4.mat REP Indi
Name_res = sprintf('MOFDA_DTLZ%d.mat',probnum+7);
save(Name_res,'REP', 'Indi')
end
% figure(002)
% title('GD')
% plot(1:maxiter,GD)
...
% figure(008)
% title('GD_01')
% plot(1:maxiter,GD_01)
% figure(009)
% title('EPS')
% plot(1:maxiter,EPS)
% figure(010)
% title('DELTA')
% plot(1:maxiter,DELTA)
% figure(011)
% title('HV')
% plot(1:maxiter,HV)
% save Kursawe_f.mat REP.pos REP.pos_fit  
