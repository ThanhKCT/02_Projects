...
load('X1_X2.mat');
fun    = @Sap_BD_HL_v3;
   %X1: pile type, X2: inclined-pile ratio, X3: pile length
lb=[1            6   16];
ub=[size(data,1) 8   39];
st=[1            1   0.1];
...
fprintf('[E-MOSFOA BD] Run mode: %s; Npop=%d; Max_it=%d; expected FEs=%d.\n', ...
...
ParallelConfig = struct( ...
    ...
    'PopulationSize', Npop);
fprintf('[E-MOSFOA BD] Using %d SAP2000 instances (%.0f%% of %d max logical workers; physical cores: %d).\n', ...
...
RunInfo = struct;
RunInfo.Algorithm = 'E-MOSFOA';
RunInfo.CaseStudy = 'BD';
...
RunInfo.FEDefinition = 'One candidate evaluated by Sap_BD_HL_v3 counts as one FE.';
...
attachDirs = {scriptDir, fullfile(scriptDir,'Functions'), fullfile(scriptDir,'BD_Sap'), fullfile(scriptDir,'Pile_TCVN10304_2014')};
attachedFiles = {};
...
for iloop=1:Nrun
    ...
    History{iloop}.Hypervolume = nan(Max_it+1,1);
...
REP{iloop}.Xpos                 = Xpos(~DOM2,:); % Store nondominated candidate solutions
...
hv(iloop,1) = hypervolume(REP{iloop}.Fitness,[1 1]);
History{iloop}.Hypervolume(1) = hv(iloop,1);
...
while T <= Max_it
    ...
    hv(iloop,historyIndex) = hypervolume(REP{iloop}.Fitness,[1 1]);
    History{iloop}.Hypervolume(historyIndex) = hv(iloop,historyIndex);
    plotting(REP{iloop},Fitness)
...
RunSummary(iloop).FeasibleFinalSolutions = sum(finalDiagnostics(:,20) > 0.5);
RunSummary(iloop).AllFinalSolutionsFeasible = all(finalDiagnostics(:,20) > 0.5);
end % end Nrun
...
Name_res = sprintf('BD_EMOSFOA_%s_%s_WF%03d_Npop%d_Nvar%d_Maxit%d.mat', ...
    upper(runMode),timestamp,round(100*workerFraction),Npop,nVar,Max_it);
save(Name_res,'REP','hv','History','EvaluationLog','FinalValidation', ...
